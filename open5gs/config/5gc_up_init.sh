...
elif [[ "$COMPONENT_NAME" =~ ^upf_test$ ]]; then
    ip tuntap add name ogstun mode tun
    ip addr add 10.47.0.1/16 dev ogstun
    ip link set ogstun up
    iptables -t nat -A POSTROUTING -s 10.47.0.1/16 ! -o ogstun -j MASQUERADE

    iperf3 -B 10.47.0.1 -s -fm &

    cp /open5gs/install/etc/open5gs/temp/upf_test.yaml  /open5gs/install/etc/open5gs/upf.yaml
